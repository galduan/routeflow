...
import { orderItemSchema } from "@/validations/order";
...
export function OrderTemplateDialog({ customerId, customerName, open, onOpenChange }: OrderTemplateDialogProps) {
  ...
  const { data: currentTemplate, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/api/customers/${customerId}/template`);
      return res.data;
    },
    enabled: open,
  });
  ...
  const saveTemplate = useMutation({
    mutationFn: async (data: TemplateInput) => {
      const res = await api.post(`/api/customers/${customerId}/template`, data);
      return res.data;
    },
    onSuccess: () => {
                      ...
                      <FormField
                        ...
                        render={({ field: f }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input type="number" {...f} onChange={(e: any) => f.onChange(Number(e.target.value))} />
                            </FormControl>
                          </FormItem>
                        )}
                      />
